...
import { checkoutUpdatedWebhookDefinition } from "./webhook-definition";

const logger = createLogger("CheckoutUpdated route");
const useCase = new TrackCheckoutUseCase(repoImpl);

const handler = checkoutUpdatedWebhookDefinition.createHandler(async (_req, ctx) => {
  try {
    ...
    logger.error("CHECKOUT_UPDATED handler threw", { message: String(error) });
